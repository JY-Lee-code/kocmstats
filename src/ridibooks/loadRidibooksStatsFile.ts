...
export const loadRidibooksStatsFile = async (filePath: string, platform: 'naver' | 'kakao' | 'ridibooks' | 'ridibooks-monthly'): Promise<StatsData[]> => {
  ...
  for (let i = 0; i < csvRows.length; i++) {
    ...
    // 판매액 가져오기
    let revenue1 = parseFloat(row['판매액'] || '0') || 0;
    let revenue2 = parseFloat(row['취소액'] || '0') || 0;
